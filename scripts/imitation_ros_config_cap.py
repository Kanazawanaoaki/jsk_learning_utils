...
class PR2Imitation(object):
  def __init__(self, comp_model, pred_model, z_dim, hidden_size, data_length, config):
    ...
    self.original_img_pub = rospy.Publisher("/imitation/original_img", sensor_msgs.msg.Image, queue_size=1)
  ...
  def get_angle_from_joint_states(self, joint_states_msg):
    joint_angles = []
    for j_dict in self.control_joints:
      idx = joint_states_msg.name.index(j_dict["name"])
      if j_dict["type"] == "revolute":
        joint_angles.append(joint_states_msg.position[idx])
      elif j_dict["type"] == "prismatic":
        joint_angles.append(joint_states_msg.position[idx])
      elif j_dict["type"] == "continuous":
        if j_dict["clamp"]:
          joint_angles.append(self.clamp_rad(joint_states_msg.position[idx]))
        else:
          joint_angles.append(joint_states_msg.position[idx])
      else:
  ...
  def execute(self):
    ...
    while True:
      ...
      original_img_cv2 = cv2.cvtColor(img_rgb.astype(np.uint8), cv2.COLOR_RGB2BGR)
      original_img_msg = self.bridge.cv2_to_imgmsg(original_img_cv2, encoding='bgr8')
      original_img_msg.header.stamp = rospy.Time.now()
      self.original_img_pub.publish(original_img_msg)
